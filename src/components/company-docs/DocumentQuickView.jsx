...
/**
 * Quick-view modal for a company document.
 * Shows the file, metadata, download, and a "Go to record" link if applicable.
 */
export default function DocumentQuickView({ doc, open, onOpenChange, ownerLabel, onGoToRecord, goToRecordLabel }) {
  if (!doc) return null;
      ...
      <DialogContent className="bg-card border-border sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader className="mb-4">
          <DialogTitle className="font-display text-foreground flex items-center gap-2.5">
            ...
            <span className="truncate">{buildDocTitle(doc.document_type, ownerLabel)}</span>
          </DialogTitle>
        </DialogHeader>
        ...
        {doc.file_url ? (
          <div className="rounded-xl overflow-hidden border border-border mb-4 bg-background/50">
            {isImage ? (
              <img src={doc.file_url} alt={doc.document_type} className="w-full max-h-[400px] object-contain" />
            ) : (
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <MetaItem icon={FileText} label="Type" value={doc.document_type} />
          <MetaItem icon={User} label="Owner" value={ownerLabel || 'Company'} />
          {doc.reference_number && <MetaItem icon={FileText} label="Reference #" value={doc.reference_number} />}
          {doc.issuing_authority && <MetaItem icon={FileText} label="Issuing Authority" value={doc.issuing_authority} />}
        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          {onGoToRecord && (
            <Button variant="outline" onClick={onGoToRecord} className="flex-1 gap-2 border-border">
              <ExternalLink className="w-4 h-4" /> {goToRecordLabel || 'Go to Record'}
            </Button>
          )}
        </div>
      </DialogContent>
