...
import { smartAllocateInvoiceNumbers } from '@/lib/invoiceSequence';
...
export default function SequenceErrorBanner({ errors, onAllocated, currentUser }) {
  ...
  const handleSmartAllocate = async () => {
    ...
    try {
      ...
      const result = await smartAllocateInvoiceNumbers(year);
      const changed = result.updates.length;
      if (changed === 0) {
        toast({ title: 'Already sequential', description: 'All invoice numbers are in correct chronological order.' });
      } else {
        // Log the smart allocation as an InvoiceNumberChange for audit
        try {
          ...
          await base44.entities.InvoiceNumberChange.create({
            ...
            reason: `Smart Allocator: renumbered ${changed} invoices to match chronological order`,
            changed_by: me,
    ...
    <div className="mb-4 rounded-xl border border-amber-500/30 bg-amber-500/8 px-4 py-3 flex items-start gap-3 animate-fade-in">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          {errors.length} invoice{errors.length !== 1 ? 's have' : ' has'} a number that doesn't match its chronological order.
          Use the Smart Allocator to renumber all invoices sequentially by creation date.
        </p>
        {errors.length <= 3 && (
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        >
          {allocating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
