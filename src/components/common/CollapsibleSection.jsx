...
export default function CollapsibleSection({ title, icon: Icon, accent = '#3b82f6', count, actions, defaultOpen = false, children }) {
  ...
  return (
    <div className="glass-card rounded-2xl p-5 animate-fade-in-up transition-all duration-300 relative overflow-hidden" style={{ borderLeft: `4px solid ${accent}` }}>
      ...
      <div className="flex items-center justify-between gap-3 relative">
        <button onClick={() => setOpen(!open)} className="flex items-center gap-2 flex-1 min-w-0 text-left">
          <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform duration-300 flex-shrink-0 ${open ? '' : '-rotate-90'}`} />
          {Icon && (
      ...
      <div className="overflow-hidden transition-all duration-500 ease-in-out" style={{ maxHeight: open ? '6000px' : '0', opacity: open ? 1 : 0 }}>
