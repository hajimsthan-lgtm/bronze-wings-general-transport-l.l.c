 ...
 * Display-only — the parent form handles auto-calculation logic.
 */
export default function TaxPreview({ subtotal, vatRate, vatAmount, total }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-muted/20 p-3.5 space-y-2.5">
      <div className="flex items-center gap-2 pb-1">
        ...
        <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Tax Breakdown — Live Preview</span>
      </div>
      ...
      <div className="flex items-center justify-between text-sm">
        <span className="text-muted-foreground flex items-center gap-1.5">
          VAT
          <span className="inline-flex items-center px-1.5 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[10px] font-semibold">
            {rate}%
          </span>
        </span>
        <span className="text-amber-400 font-medium tabular-nums">+ {formatCurrency(vat)}</span>
      </div>
