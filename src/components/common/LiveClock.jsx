...
export default function LiveClock() {
  ...
  return (
    <div
      className="hidden lg:flex flex-col justify-center gap-0.5 h-9 pl-2 pr-3 rounded-full transition-all duration-300"
      style={{
    ...
    >
      <div className="flex items-center gap-2">
        ...
        {/* time */}
        <span className="font-mono text-[12px] font-bold tabular-nums tracking-tight text-white/90 leading-none">{hh}:{mm}</span>
        {/* AM/PM badge */}
        <span className="text-[8px] font-bold tracking-[0.16em] px-1.5 py-0.5 rounded-full leading-none"
      ...
      {/* date — aligned below the time */}
      <span className="text-[8.5px] font-medium tracking-[0.12em] text-white/45 leading-none pl-6">{date}</span>
    </div>
  );
}