...
function MetaItem({ icon: Icon, label, value, tone }) {
  return (
    <div className="flex items-center gap-1.5 min-w-0">
      ...
      <span className="text-[10px] uppercase tracking-wider text-muted-foreground/70 whitespace-nowrap">{label}</span>
      <span className={`text-xs truncate ${tone || 'text-foreground'}`}>{value || '—'}</span>
    </div>
  );
}
    ...
    <div className="entity-card cursor-pointer group" onClick={onOpen}>
      <div className="flex items-start justify-between mb-3.5">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-11 h-11 rounded-xl entity-avatar flex items-center justify-center relative">
            <Truck className="w-5 h-5 text-white/70" />
            <span className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full ring-2 ring-[#1a1d29]" style={{ background: accent, boxShadow: `0 0 8px ${accent}` }} />
          </div>
