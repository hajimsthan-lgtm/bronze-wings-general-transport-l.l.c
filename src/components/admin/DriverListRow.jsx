...
import { Pencil, Trash2, ChevronRight, Phone, Check } from 'lucide-react';
...
export default function DriverListRow({ d, onOpen, onEdit, onDelete, selected = false, onSelect }) {
  const { t } = useI18n();
  const [confirmDel, setConfirmDel] = useState(false);

  return (
    <>
      <div className={`row-card row-edge-glow flex items-start gap-3 cursor-pointer group ${selected ? 'ring-1 ring-primary/50' : ''}`} onClick={() => onOpen?.(d)} style={{ ['--row-accent']: ACCENT }}>
        <div
