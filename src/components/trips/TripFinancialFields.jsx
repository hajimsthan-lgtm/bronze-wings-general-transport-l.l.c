...
export default function TripFinancialFields({ p }) {
  ...
  const isContract = form.trip_type === 'contract';
  return (
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          {revenueOverridden ? (
            ...
            <p className="text-[10px] text-primary mt-1">{isContract ? 'Direct entry — no overtime calc' : 'Auto-calculated: base fare + overtime (editable)'}</p>
          )}
        </div>
