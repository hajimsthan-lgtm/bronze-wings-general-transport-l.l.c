...
export default function FinanceStatCards({ invoices }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      <KpiCard icon={Banknote} title="Cash Balance" value={formatCurrency(cash)} accent="amber" />
      <KpiCard icon={Landmark} title="Bank Balance" value={formatCurrency(bank)} accent="teal" />
      <KpiCard icon={TrendingUp} title="Revenue (Paid)" value={formatCurrency(revenue)} accent="emerald" />
      <KpiCard icon={Clock} title="Outstanding" value={formatCurrency(outstanding)} accent="red" />
    </div>
  );
}